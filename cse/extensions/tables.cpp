...
ASCIITableWindow::ASCIITableWindow()
  : WindowProcess("ASCII table")
{
}

void ASCIITableWindow::render()
{
  constexpr int charCount = 128, colCount = 4, rowCount = charCount / colCount;
  if (ImGui::BeginTable("##asciitable", colCount)) {
    for (int i = 0; i < charCount; i++) {
      if (i % colCount == 0)
        ImGui::TableNextRow();
      ImGui::TableNextColumn();
      int c = i % colCount * rowCount + (i / colCount);
      ImGui::Text("%03d 0x%02x %c", c, c, c);
    }
    ImGui::EndTable();
  }
}

GLSLTableWindow::GLSLTableWindow()
  : WindowProcess("GLSL table")
{
}

void GLSLTableWindow::render()
{
...
void ColorsSchemesWindow::render()
{
...
std::string ColorsSchemesWindow::formatColorStr(int color)
{
  std::string processed = m_userFormat;
  replaceFirst(processed, "$R", std::to_string((color >> 16) & 0xff));
  replaceFirst(processed, "$r", std::to_string(static_cast<float>((color >> 16) & 0xff) / 0xff));
  replaceFirst(processed, "$G", std::to_string((color >> 8) & 0xff));
  replaceFirst(processed, "$g", std::to_string(static_cast<float>((color >> 8) & 0xff) / 0xff));
  replaceFirst(processed, "$B", std::to_string((color >> 0) & 0xff));
  replaceFirst(processed, "$b", std::to_string(static_cast<float>((color >> 0) & 0xff) / 0xff));
  return processed;
}

void ColorsSchemesWindow::replaceFirst(std::string& toAffect, const char* toReplace, const std::string &replacement)
{
...
void ColorsSchemesWindow::drawColorScheme(const int* scheme)
{
...
void ColorsSchemesWindow::drawColorBtn(int color)
{
  ImVec4 c{
    static_cast<float>(color >> 16 & 0xff) / 0xff,
    static_cast<float>(color >> 8  & 0xff) / 0xff,
    static_cast<float>(color >> 0  & 0xff) / 0xff,
    1
  };
...
void ColorsSchemesWindow::drawAnsiBtn(const char* colorName, int foregroundCode) const
{
  if (ImGui::Button(colorName))
    ImGui::SetClipboardText((ANSI_CSI[m_ansiCSI] + std::to_string(foregroundCode) + "m").c_str());
}

void ColorsSchemesWindow::drawAnsiBtn(const char* colorName, int foregroundCode, int color) const
{
...
Tables::Tables()
{
  ...
  for (const auto &[name, _] : m_tables)
    tableNames.emplace_back(name);

  cse::commands::addCommand({
    "table",
    "sheet sheets & tables",
    { std::make_shared<CommandEnumPart>("name", tableNames)},
    [this](auto &parts) {
      for (const auto &[name, tableProvider] : m_tables) {
        if (parts[0] == name) {
          graphics::createWindow(std::shared_ptr<WindowProcess>{ tableProvider() });
          return;
        }
      }
    }
